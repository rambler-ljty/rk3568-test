/*
 * DS18B20温度传感器Linux内核驱动模块 - 温度精度配置和读取版本
 * 功能：实现DS18B20温度传感器的完整应用，集成GPIO控制、通信协议、温度读取、精度配置和精度读取功能
 * 包含DS18B20复位时序、读写操作、温度转换、温度读取、精度设置、精度验证、精度读取等完整功能
 */
...
/**
 * 温度精度配置相关的宏定义
 * 使用ioctl命令控制DS18B20的温度精度设置和读取
 */
#define SET_RESOLUTION _IOW('A',0,int)   // ioctl命令：设置温度精度，'A'为命令类型，0为命令编号，int为参数类型
#define READ_RESOLUTION _IOR('A',1,int)  // ioctl命令：读取温度精度，'A'为命令类型，1为命令编号，int为参数类型

// 温度精度选项定义
#define SET_RESOLUTION_9  9   // 9位精度：0.5°C分辨率，转换时间93.75ms
#define SET_RESOLUTION_10 10  // 10位精度：0.25°C分辨率，转换时间187.5ms
#define SET_RESOLUTION_11 11  // 11位精度：0.125°C分辨率，转换时间375ms
#define SET_RESOLUTION_12 12  // 12位精度：0.0625°C分辨率，转换时间750ms
...
/**
 * set_resolution - 设置DS18B20温度精度函数
 * @args: 精度参数（9、10、11或12）
 * 功能：通过写暂存器命令设置DS18B20的温度精度
 * 
 * DS18B20精度设置流程：
 * 1. 发送复位信号
 * 2. 发送跳过ROM命令（0xCC）
 ...
 * 
 * 精度配置值说明：
 * - 9位精度：0x1F，分辨率0.5°C，转换时间93.75ms
 * - 10位精度：0x3F，分辨率0.25°C，转换时间187.5ms
 * - 11位精度：0x5F，分辨率0.125°C，转换时间375ms
 * - 12位精度：0x7F，分辨率0.0625°C，转换时间750ms
 */
void set_resolution(int args){
	...
	// 3. 发送写暂存器命令（0x4E），准备写入配置数据
	ds18b20_writebyte(0x4E);
	
	// 4. 写入TH寄存器值（60），温度上限报警值
	ds18b20_writebyte(60);
	
	// 5. 写入TL寄存器值（10），温度下限报警值
	ds18b20_writebyte(10);

	// 6. 根据精度参数写入配置寄存器值
	switch(args){
	
		case SET_RESOLUTION_9:   // 9位精度
			ds18b20_writebyte(0x1F);  // 配置值0x1F
			break;
		case SET_RESOLUTION_10:  // 10位精度
			ds18b20_writebyte(0x3F);  // 配置值0x3F
			break;
		case SET_RESOLUTION_11:  // 11位精度
			ds18b20_writebyte(0x5F);  // 配置值0x5F
			break;
		case SET_RESOLUTION_12:  // 12位精度
			ds18b20_writebyte(0x7F);  // 配置值0x7F
			break;
		default:  // 无效精度参数
			break;
	}
...
/**
 * check_args - 验证DS18B20温度精度设置函数
 * @args: 精度参数（9、10、11或12）
 * 功能：读取DS18B20的配置寄存器，验证精度设置是否成功
 * 返回值：0表示设置成功，-1表示设置失败
 * 
 * 验证流程：
 * 1. 发送复位信号
...
int check_args(int args){
	...
	// 3. 发送读取暂存器命令（0xBE），准备读取配置数据
	ds18b20_writebyte(0xBE);
	
	// 4. 跳过前4个字节，定位到配置寄存器
	ds18b20_readbyte();  // 跳过TH寄存器
	ds18b20_readbyte();  // 跳过TL寄存器
	ds18b20_readbyte();  // 跳过配置寄存器
	ds18b20_readbyte();  // 跳过保留字节

	// 5. 读取配置寄存器值，验证精度设置
	switch(args){
		case SET_RESOLUTION_9:   // 验证9位精度
			if(ds18b20_readbyte()==0x1F){  // 检查是否为0x1F
				ret=0;  // 设置成功
			}
			break;
		case SET_RESOLUTION_10:  // 验证10位精度
			if(ds18b20_readbyte()==0x3F){  // 检查是否为0x3F
				ret=0;  // 设置成功
			}
			break;
		case SET_RESOLUTION_11:  // 验证11位精度
			if(ds18b20_readbyte()==0x5F){  // 检查是否为0x5F
				ret=0;  // 设置成功
			}
			break;
		case SET_RESOLUTION_12:  // 验证12位精度
			if(ds18b20_readbyte()==0x7F){  // 检查是否为0x7F
				ret=0;  // 设置成功
			}
			break;
		default:  // 无效精度参数
...
/**
 * read_resolution - 读取DS18B20温度精度函数
 * 功能：读取DS18B20的配置寄存器，获取当前设置的温度精度
 * 返回值：配置寄存器值，表示当前的精度设置
 * 
 * 读取流程：
 * 1. 发送复位信号
 * 2. 发送跳过ROM命令（0xCC）
 ...
 * 返回值说明：
 * - 0x1F：9位精度，分辨率0.5°C
 * - 0x3F：10位精度，分辨率0.25°C
 * - 0x5F：11位精度，分辨率0.125°C
 * - 0x7F：12位精度，分辨率0.0625°C
 */
int read_resolution(void){

	int ret;  // 存储读取到的配置寄存器值
...
/**
 * ds18b20_ioctl - 字符设备控制函数
 * @file: 文件结构体指针
 * @cmd: 控制命令
 * @args: 命令参数
 * 返回值：0表示成功，-1表示失败
 * 
 * 功能：实现用户空间对DS18B20的控制接口
 * 支持温度精度设置和读取等高级功能
 * 
 * 支持的命令：
 * - SET_RESOLUTION：设置温度精度（9、10、11或12位）
 * - READ_RESOLUTION：读取当前温度精度设置
 */
long ds18b20_ioctl(struct file *file,unsigned int cmd,unsigned long args){

	int resolution;  // 存储读取到的精度值
...
struct file_operations ds18b20_fops={
	...
	.unlocked_ioctl=ds18b20_ioctl,        // 设备控制
